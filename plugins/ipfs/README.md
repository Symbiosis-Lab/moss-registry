...
No other hosts are contacted. The named binary grants (`execute_binary:ipfs`,
`execute_binary:sh`) cover detecting and starting an already-installed Kubo
(`ipfs version` / `ipfs init`, and `sh` only for the detached `ipfs daemon`
spawn) — the plugin never downloads binaries. Site content is uploaded only to the provider(s) the user
configured.
