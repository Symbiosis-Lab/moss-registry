...
describe("deploy — reachability confirmation", () => {
  ...
  it("shows the ordinary success toast when the receiver confirms reachability", async () => {
    mockWaitForReachability.mockResolvedValue({ reachable: true, httpCode: "301" });
    ...
    expect(result.success).toBe(true);
    expect(mockShowToast).toHaveBeenCalledWith(
      expect.objectContaining({ message: "Published to your onion site.", variant: "success" }),
    );
    expect(result.deployment!.metadata.onion_reachable).toBe("true");
  });

  it("softens the toast, without claiming failure, when the receiver confirms unreachability", async () => {
    mockWaitForReachability.mockResolvedValue({ reachable: false, httpCode: "takeover" });

    const result = await deploy(CONTEXT);

    // The publish itself still succeeded — only the reachability claim changes.
    expect(result.success).toBe(true);
    expect(mockShowToast).toHaveBeenCalledWith(
      expect.objectContaining({
        variant: "info",
        message: expect.stringContaining("not reachable on Tor yet"),
        actions: [{ label: "View site", url: "http://abcdef1234567890.onion/" }],
      }),
    );
    expect(result.deployment!.metadata.onion_reachable).toBe("false");
  });

  it("does not overclaim when the poll window elapses without an answer (older receiver or slow check)", async () => {
    mockWaitForReachability.mockResolvedValue({ reachable: null, httpCode: null });

    const result = await deploy(CONTEXT);

    expect(result.success).toBe(true);
    // Falls back to today's copy rather than showing an unreachable warning
