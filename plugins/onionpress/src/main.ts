...
async function deploy(_context: DeployContext): Promise<HookResult> {
  ...
  try {
    ...
    console.log(
      `   Published: ${onionUrl}` +
        (reachable === false ? ` (not yet reachable on Tor: ${httpCode})` : ""),
    );

    if (reachable === false) {
      await showToast({
        message:
          "Published, but not reachable on Tor yet — the address should resolve within a minute.",
        variant: "info",
        actions: [{ label: "View site", url: onionUrl }],
        duration: 8000,
      });
    } else {
      // reachable === true, or null (older receiver / poll window elapsed
      // without an answer) — in both cases don't claim a negative that
      // isn't confirmed; today's copy stands.
      await showToast({
        message: "Published to your onion site.",
        variant: "success",
        actions: [{ label: "View site", url: onionUrl }],
        duration: 8000,
      });
    ...
    return {
      success: true,
      message: `Your site is live on Tor at ${onionUrl}`,
      deployment: {
        method: "onionpress",
        url: onionUrl,
